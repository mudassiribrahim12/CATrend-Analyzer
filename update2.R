...
server <- function(input, output, session) {
  ...
  perform_cochran_armitage_accurate <- function(cont_table, test_type = "two.sided", continuity_correction = FALSE) {
    ...
    # Calculate chi-square statistic (z^2) - THIS IS THE CORRECT CHI-SQUARE FOR THE TEST
    chi_square <- z^2
    
    # Calculate chi-square p-value (1 degree of freedom)
    chi_square_p_value <- pchisq(chi_square, df = 1, lower.tail = FALSE)
  ...
  # Test results - CONSISTENT OUTPUT WITH CORRECT CHI-SQUARE
  output$test_results <- renderPrint({
    ...
    chi_sq_p_value <- results$test_results$chi_square_p_value
    one_sided_p <- results$test_results$p.value
    two_sided_p <- results$two_sided_test$p.value
    
    # Always display Z-statistic, Chi-square, and degrees of freedom
    cat("Z-statistic:", format(z_stat, digits = 4), "\n")
    cat("Chi-square statistic:", format(chi_sq, digits = 4), "\n")
    cat("Degrees of freedom: 1\n")
    cat("Chi-square p-value:", format(chi_sq_p_value, digits = 4), "\n\n")
    
    # Display appropriate p-values based on Z-statistic sign
    if (results$is_increasing) {
      cat("One-sided p-value (increasing):", format(one_sided_p, digits = 4), "\n")
      cat("Two-sided p-value:", format(two_sided_p, digits = 4), "\n")
    } else if (results$is_decreasing) {
      cat("One-sided p-value (decreasing):", format(one_sided_p, digits = 4), "\n")
      cat("Two-sided p-value:", format(two_sided_p, digits = 4), "\n")
    } else {
      cat("Two-sided p-value:", format(two_sided_p, digits = 4), "\n")
    }
    ...
    if (p_val_interpret < 0.05) {
      cat("✓ Statistically significant trend detected (p =", 
          format(p_val_interpret, digits = 3), ")\n")
      if (results$is_increasing) {
    ...
    if (p_val_interpret < 0.001) {
      cat("*** p < 0.001 (Highly significant)\n")
    } else if (p_val_interpret < 0.01) {
      cat("** p < 0.01 (Very significant)\n")
    } else if (p_val_interpret < 0.05) {
      cat("* p < 0.05 (Significant)\n")
    } else if (p_val_interpret < 0.1) {
  ...
  output$report_preview <- renderUI({
    ...
    HTML(paste(
      ...
      "<p><strong>Z-statistic:</strong>", format(z_stat, digits = 4), "</p>",
      "<p><strong>Chi-square statistic:</strong>", format(chi_sq, digits = 4), "</p>",
      "<p><strong>Chi-square p-value:</strong>", format(chi_sq_p_value, digits = 4), "</p>",
      if (results$is_increasing) {
        paste0("<p><strong>One-sided p-value (increasing):</strong> ", format(one_sided_p, digits = 4), "</p>")
      } else if (results$is_decreasing) {
        paste0("<p><strong>One-sided p-value (decreasing):</strong> ", format(one_sided_p, digits = 4), "</p>")
      } else {
        ""
      },
      "<p><strong>Two-sided p-value:</strong> ", format(two_sided_p, digits = 4), "</p>",
    ...
    content = function(file) {
      ...
      tryCatch({
        ...
        rmd_content <- paste0(
          ...
          "- **Chi-square p-value:** ", format(chi_sq_p_value, digits = 4), "\n",
          if (results$is_increasing) {
            paste0("- **One-sided p-value (increasing):** ", format(one_sided_p, digits = 4), "\n")
          } else if (results$is_decreasing) {
            paste0("- **One-sided p-value (decreasing):** ", format(one_sided_p, digits = 4), "\n")
          } else {
            ""
          },
          "- **Two-sided p-value:** ", format(two_sided_p, digits = 4), "\n\n",
          ...
          if (results$is_increasing && one_sided_p < 0.05) {
            paste0("There is a statistically significant **INCREASING** trend in the proportion of **'", 
                   first_dep_level, "'** across the ordered levels of **", 
                   ind_var, "** (Z = ", format(z_stat, digits = 3), ", χ² = ", format(chi_sq, digits = 3), 
                   ", df = 1, p = ", format(one_sided_p, digits = 4), "). ",
          ...
          } else if (two_sided_p < 0.05) {
            paste0("There is a statistically significant relationship between **", 
                   dep_var, "** and **", ind_var, "** (χ² = ", format(chi_sq, digits = 3), 
                   ", df = 1, p = ", format(two_sided_p, digits = 4), "). ",
            ...
            paste0("No statistically significant trend was detected in the proportion of **",
                   ...
                   ind_var, "** (χ² = ", format(chi_sq, digits = 3), ", df = 1, p = ", 
                   format(one_sided_p, digits = 4), "). ",
          ...
          "The chi-square statistic is calculated as Z² with 1 degree of freedom.\n\n",
